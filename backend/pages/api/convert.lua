...
local fh = io.open("/var/www/foxcaves/tmp/files/" .. rgs.fileid .. dbdata.extension, "w")
fh:write(data)
fh:close()
os.execute('/usr/bin/convert "/var/www/foxcaves/tmp/files/' .. rgs.fileid .. dbdata.extension .. '" -format ' .. newextension:sub(2) .. ' "/var/www/foxcaves/tmp/files/' .. rgs.fileid .. newextension .. '"')
os.remove("/var/www/foxcaves/tmp/files/" .. args.fileid .. dbdata.extension)

dofile("scripts/mimetypes.lua")

local newsize = lfs.attributes("/var/www/foxcaves/tmp/files/" .. rgs.fileid .. newextension, "size")
if not newsize then
	ngx.status = 500
	ngx.print("failed")
	return ngx.eof()
...
file_push_action(args.fileid, 'refresh')
