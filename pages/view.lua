...
local file = database:query("SELECT f.fileid, f.thumbnail, f.type, f.name, u.username, f.time, f.extension, f.size FROM files AS f, users AS u WHERE f.fileid = '"..nameregex.."' AND f.user = u.id LIMIT 0,1")
...
ngx.print(load_template("view", {MAINTITLE = "View file - " .. file.name, ADDLINKS = build_nav(navtbl), FILE = file}))
ngx.eof()
