...
%>
<li draggable="true" id="file_<%= fileid %>" class="image_manage_main" style="background-image:url('<% if file.type == 1 and file.thumbnail and file.thumbnail ~= "" then %>https://f0x.es/thumbs/<%= fileid %><%= file.thumbnail %><% elseif G.lfs.attributes("static/img/thumbs/ext_" .. file.extension .. ".png", "size") then %><%= STATIC_URL_PREFIX %>/img/thumbs/ext_<%= file.extension %>.png<% else %><%= STATIC_URL_PREFIX %>/img/thumbs/nothumb.png<% end %>')">
	...
	<div class="image_manage_bottom">
		<span>
			...
			<a title="Download" href="https://f0x.es/d<%= fileid %><%= file.extension %>"><i class="icon-download icon-white"></i> </a>
			<div class="dropdown">
