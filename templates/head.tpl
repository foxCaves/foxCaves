...
local totalbytes = G.tonumber(G.ngx.ctx.user.totalbytes)
...
<% end %>
						<li class="dropdown">
							<a href="" class="dropdown-toggle" data-toggle="dropdown">Welcome, <% if G.ngx.ctx.user then %><%= G.ngx.ctx.escape_html(G.ngx.ctx.user.username) %><% else %>Guest<% end %> <b class="caret"></b></a>
